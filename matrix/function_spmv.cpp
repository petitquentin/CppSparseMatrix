...
//Matrix values : double AND denseVector : double
    ...
    if(my_rank == 0){
        ...
        if(MNL1 == sizeDenseVector){
            ...
            for(int i = 1; i < p; i++){
                ...
                MPI_Isend(&row[(int)(i * ceil(rowSize*1.0/p))], sizeData, MPI_LONG, i, TAGROW, MPI_COMM_WORLD, &req);
                MPI_Isend(&val[(int)(i * ceil(rowSize*1.0/p))], sizeData, MPI_DOUBLE, i, TAGVAL, MPI_COMM_WORLD, &req);
                MPI_Isend(&col[(int)(i * ceil(rowSize*1.0/p))], sizeData, MPI_LONG, i, TAGCOL, MPI_COMM_WORLD, &req);
            }
        ...
        for(int i = 0; i < sizeY; i++){
            for(int j = 0; j < sizeX; j++){
                if(ai[i*sizeX+j] != -1){
                    ...
                    int nombre = ai[i*sizeX + j]*maxInd + jc[i*sizeX + j];
        ...
        for(int i = 0; i < sizeY; i++){
            for(int j = 0; j < sizeX; j++){
                if(aj[i*sizeX+j] != -1){
                    ...
                    MPI_Recv(&recep, 1, MPI_DOUBLE, floor(ic[i*sizeX + j]/yGrid) * (MNL1/xGrid) + floor(aj[i*sizeX + j]/xGrid), MPI_ANY_TAG, MPI_COMM_WORLD, &status);
                    //cout <<my_rank << " : " << status.MPI_TAG << " : " << (int)(((int)(status.MPI_TAG/maxInd) - my_x)*sizeX + ((status.MPI_TAG%maxInd)-my_y)) << " (" << ((int)(status.MPI_TAG/maxInd) - my_x)*sizeX << ' ' << ((status.MPI_TAG%maxInd)-my_y) << ")" << endl;
                    tres[ (int)(((status.MPI_TAG%maxInd)-my_y)*sizeX + ((int)(status.MPI_TAG/maxInd) - my_x))] = recep;
                    //(j+my_x) * 5 + (i+my_y) *3
    ...
    *result = (double *)malloc(sizeof(double)*MNL1);
    if(my_rank == 0){
        ...
        if(MNL1 == sizeDenseVector){
            ...
            for (long int  i = 0; i < sizeData; i++){
                (*result)[row[i]] += val[i]*((double)(vec[col[i]]));
            }
    ...
    if(my_rank == 0){
        ...
        if(MNL1 == sizeDenseVector){
            ...
            if(actualProcessus >= p){
                for(int i = lastInd-1; i < MNL1; i++){
                    for(int j = 0; j < ptr[i+1]-ptr[i]; j++){
                        (*result)[i] += ((double)(denseVector[ind[j+ptr[i]]])) * val[j+ptr[i]]; 
                        //result[i] +=  denseVector[ind[j+ptr[i]-1]] * val[j+ptr[i]-1];
                    }
                }
                actualProcessus = actualProcessus - 1;
        ...
        if(MNL1 == sizeDenseVector){
            ...
            if(sizePtr != -1){
                ...
                *result = (double *)malloc(sizeof(double)*sizePtr);
                ...
                for (long int  i = 0; i < sizePtr; i++){
                    for(long int j = 0; j < ptr[i+1]-ptr[i]; j++){
                        (*result)[i] +=  ((double)(vec[ind[j+ptr[i]-ptr[0]]])) * val[j+ptr[i]-ptr[0]];
                    }
                }
                MPI_Send(&sizePtr, 1, MPI_LONG, 0, tag, MPI_COMM_WORLD);
                MPI_Send(*result, sizePtr, MPI_DOUBLE, 0, tag, MPI_COMM_WORLD);
        ...
        if(MNL1 == sizeDenseVector){
            ...
            if(actualProcessus >= p){
                for(int i = lastInd-1; i < MNL1; i++){
                    for(int j = 0; j < rowSize; j++){
                        if(indCol[j+i*rowSize] != -1){
                            (*result)[i] += ((double)(denseVector[indCol[j+i*rowSize]])) * val[j+i*rowSize];
                        }
            ...
            if(nbRowProc != -1){
                ...
                *result = (double *)malloc(sizeof(double)*nbRowProc);
                ...
                for (long int  i = 0; i < nbRowProc; i++){
                    for(long int j = 0; j < rowSize; j++){
                        if(ind[i*rowSize+j] != -1){
                            (*result)[i] +=  ((double)(vec[ind[i*rowSize+j]])) * val[i*rowSize+j];
                        }
                    }
                }
                MPI_Send(&nbRowProc, 1, MPI_LONG, 0, tag, MPI_COMM_WORLD);
    ...
    if(sizeDenseVector == MNL1){
        if(my_rank == 0){
            ...
            for(int i = 0; i < floor(maxInd/yGrid); i++){
                for(int j = 0; j < MNL1; j++){
                    if(floor(i * MNL1/xGrid) + floor(j/xGrid) != 0){
                        MPI_Send(&denseVector[j], 1, MPI_FLOAT, floor(i * MNL1/xGrid) + floor(j/xGrid), j%xGrid, MPI_COMM_WORLD);
                    }
                }
            }
            sizeX = xGrid;
            ...
            aj = (long int *)malloc(sizeof(long int) * sizeX * sizeY);
            ic = (long int *)malloc(sizeof(long int) * sizeY * sizeX);
            vecX = (float *)malloc(sizeof(float) * sizeX);
            for(int i = 0; i < sizeY; i++){
                for(int j = 0; j < sizeX; j++){
                    acr[i*sizeX + j] = acrc[i*MNL1 + j];
                    ai[i*sizeX + j] = aic[i*MNL1 + j];
            ...
            if(sizeX * sizeY != 0){
                for(int i = 0; i < sizeX; i++){
                    MPI_Recv(&vecX[i], 1, MPI_FLOAT, 0, i, MPI_COMM_WORLD, &status);
                }
            }
            /* cout << my_rank << "ACC : ";
            for(int i =0; i < sizeY * sizeX; i++){
        ...
        for(int i = 0; i < sizeY; i++){
            for(int j = 0; j < sizeX; j++){
                if(ai[i*sizeX+j] != -1){
                    //t[i*sizeX + j] += vecX[my_x+j] * acr[i*sizeX + j];
                    t[i*sizeX + j] += ((double)(vecX[j])) * acr[i*sizeX + j];
                    int nombre = ai[i*sizeX + j]*maxInd + jc[i*sizeX + j];
